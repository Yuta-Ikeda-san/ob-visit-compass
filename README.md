...
業界・職種を対話形式で聞き取り、Hugging Face経由のLLMが
...
- PostgreSQL（Render上に構築）/ SQLite（ローカル開発用フォールバック）
- Hugging Face Inference API（フォールバック方式：Qwen2.5-7B-Instructで
  応答が得られない場合はQwen2.5-72B-Instructに自動的に切り替えることで
  安定性を確保）
- Render（デプロイ・ホスティング）
...
Claude（対話型AIアシスタント）と対話しながら実装・デバッグ・デプロイを
進める、AIエージェント駆動開発（Vibe Coding）のプロセスで構築しました。
...
### 2. 使用モデルの見直し（DeepSeek-R1 → 複数モデルのフォールバック方式）
...
対策として、まず後処理で`<think>`タグを除去する関数を実装した上で、
より高速なQwen2.5-7B-Instructへ変更した。さらに安定性を高めるため、
1つのモデルが失敗・タイムアウトした場合に別のモデルへ自動的に
切り替えるフォールバック方式を導入した。
...
発生する問題があった。保存前に日本時間へ変換した上でタイムゾーン
